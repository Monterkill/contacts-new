...
public class SingleThreadProcessor implements IProcess {
    ...
    public void process(Task task) {
        ...
        if(task.getPriority() == Task.BACK){
            Thread back_t = new BackTaskThread(task);
            back_t.start();
        }else {
            task.setStatus(task._exec());
            LogBus.Log(LogBus.DEBUGTAGS, "finish one task");
        }
    }

    private class BackTaskThread extends Thread{

        private Task _task;

        public BackTaskThread(Task task){
            _task = task;
        }

        @Override
        public void run() {
            LogBus.Log(LogBus.DEBUGTAGS, "start a back task");
            _task.setStatus(_task._exec());
            LogBus.Log(LogBus.DEBUGTAGS, "finish a back task");
        }
    }
}
